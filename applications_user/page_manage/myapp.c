...
本身不含业务逻辑，只是把事件"往上交"。
这样设计让底层视图层和上层场景逻辑解耦——这是 Flipper Scene 架构的固定套路

 ------------------------- */

// 这个函数在当前的项目没有使用
// 这个函数在使用 view_dispatcher_send_custom_event 的时候触发
// 它自己不处理事件，而是把事件转交给 SceneManager，
// 由 SceneManager 派发给当前场景的 on_event 函数去处理
...
// 用户按下 Flipper 的返回键时，ViewDispatcher 回调这个函数
...
static App* app_alloc() {
    ...
    // 申请一个视图分发器（新固件 alloc 时已自动启用事件队列）
...
int32_t myapp_app(void* p) {
    UNUSED(p);
    App* app = app_alloc(); // 分配并初始化应用程序结构体
    ...
    // 运行视图分发器，开始处理用户输入和界面切换
    view_dispatcher_run(app->view_dispatcher);

    furi_record_close(RECORD_GUI); // 关闭GUI记录，配对furi_record_open
    app_free(app);
    return 0;
}
